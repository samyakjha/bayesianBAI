...
def seqHalvHistory(bandits, n):
    ...
    for round in range(R):
        ...
        for i in range(K):
            ...
            sampMean = banditlist[i].sample_mean(totalSamp_list[i], sumSamp)
            sampmean_list[i] = sampMean
        red = math.ceil(num_act/2)
        #print('r', red)
        #print('pml', postmean_list)
        tophalf_indices = np.argpartition(sampmean_list, K-red)
...
def seqHalving(bandits, n):
    ...
    for round in range(R):
        ...
        for i in range(K):
            ...
            sumSamp = banditlist[i].pullArm_sum(numSamp_list[i])
            sampMean = banditlist[i].sample_mean(numSamp_list[i], sumSamp)
            sampmean_list[i] = sampMean
        red = math.ceil(num_act/2)
        #print('r', red)
        #print('pml', postmean_list)
...
#print(bandits.K)
best = seqHalvHistory(bandits, 25)
